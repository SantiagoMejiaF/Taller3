...
import com.example.taller3.databinding.ActivityRegistroFotoVetBinding
...
class RegistroFotoVetActivity : AppCompatActivity() {


    private lateinit var binding: ActivityRegistroFotoVetBinding
    private lateinit var file: File

    @RequiresApi(Build.VERSION_CODES.Q)
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityRegistroFotoVetBinding.inflate(layoutInflater)
    ...
    private fun subirImagenFirebase(uri: Uri, callback: (Boolean) -> Unit) {
        ...
        val storageRef =
            firebaseStorage.reference.child("Fotos de perfil veterinarios").child("${System.currentTimeMillis()}.jpg")

        storageRef.putFile(uri).addOnSuccessListener { uploadTask ->
            uploadTask.storage.downloadUrl.addOnSuccessListener { downloadUri ->
                val veterinario = VeterinarioData()
                veterinario.foto = downloadUri.toString()

                val updateData = HashMap<String, Any>()
                updateData["foto"] = veterinario.foto!!

                firebaseDatabase.reference.child("Fotos de perfil veterinarios").child(firebaseAuth.currentUser!!.uid)
        ...
        if (requestCode == 2 && resultCode == AppCompatActivity.RESULT_OK) {
            ...
            subirImagenFirebase(Uri.fromFile(file)) { success ->
                if (success) {
                    Toast.makeText(
                        this,
                        "Imagen subida correctamente a Firebase",
                        Toast.LENGTH_SHORT
                    ).show()
                } else {
